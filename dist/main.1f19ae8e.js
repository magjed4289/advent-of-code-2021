...
function groupIncreases(arr) {
  ...
  for (var j = 0; j < 4; j++) {
    for (var i = 0 + j; i < n - 2; i++) {
      sum = Number(arr[i]) + Number(arr[i + 1]) + Number(arr[i + 2]); //document.write("////");
...
function secondRound(arr) {
  var myArray = groupIncreases(arr);
  var number = myArray.length;
  document.write("///////THIS IS THE NUMBER OF CALCS: " + number);
  countIncreases(myArray, number);
}
